...
> `le` has the largest population SD. This implies that this process has higher variability meaning that it fluctuates more widely than `gpp` or `reco`. This might also suggest that `le` responds more strongly to various changes in environment than `gpp` or `reco`. 
...
> `le` has the largest relative error at my chose `n_samp`. Some variables are harder to estimate from a small sample because they have higher variability within the population, as previously seen when `le` was observed to have the highest population SD. This could be due to the fact the latent heat might impacted by more different varaibles than either `gpp` or `reco. 
...
p_relerr <-  ggplot(summary_relerr, aes(x = n)) +
  geom_line(aes(y = median_relerr), color = "darkred") +
  geom_point(aes(y = median_relerr), color = "darkred") +
  labs(
    x = "Sample size (n)",
    y = "Relative Error (%)",
    title = "Relative Error vs Sample Size for le"
  )+
  theme_bw()
...
> As sample size increases, relative error decreases. The improvement from n=25→100 is larger than 300→1000, as you can see the slope becoming shallower.  Accuracy shows 'diminishing returns' as n increases because once the sample is fairly representative of the population, additional samples result in smaller improvements. 
...
tibble(mean_est = means) %>%
  ...
  geom_histogram(bins = 10, alpha = 0.85, fill="darkred") +
...
> The `se_theory` (3.603047) and `se_simulated`() were fairly close to each other. If I increase `n_samp` the standard error decreases. If I  increase `reps` `se_simulated` becomes closer to `se_theory`. 
...
ggplot(tibble(boot_means = boot_means), aes(x = boot_means)) +
  geom_histogram(bins = 30, fill = "darkred") +
  geom_vline(xintercept = mean(samp$x), linetype = "dashed", size = 1) +
  labs(
    x = "Bootstrap Means",
    y = "Frequency",
    title = "Bootstrap Distribution of Mean for le"
  ) +
  theme_minimal()
```
...
> The bootstrap distribution is fairly similar to the sampling distribution from Part III. They are both approximately normal. The bootstrap is trying to approximate the sampling distribution. In EAES, population data can be hard to collect, limited, or extremely skewed. Boostrap is especially useful when you do not have access to the entire population or when it is higly variable or skewed. 
...
tibble(boot_mean = boot_means_random) %>%
  ...
  geom_histogram(bins = 30, alpha = 0.85, fill="darkred") +
...
> The bootstrap distribution is not exactly centered near the population mean. However, the sample mean is not that far off from the population mean. The bootstrap SD represents the standard deviation or uncertainty of the sample mean. 
...
> The bootstrap distribution looks "confident"/narrow even though it is centered far from the population mean. In this example, precision refers to the narrowness of the bootstrap distribution. Accuracy refers to how close the estimate or sample mean is to the population mean. The summer only sampling introduces systematic error because this way of sampling consistently excludes the other seasons, shifting the estimate of the mean/ the sample distribution away from the population mean. 
...
> Both the random bootstrap distribution and the stratified distribution are centered near the population mean, but not exactly on it. The summer-only distribution is shifted far away from the population mean. The stratified sampling design is most accurate for the annual mean with the sample mean being fairly close to the population mean and th SD being smaller. The summer-only sampling design is most precise as the distribution is narrower than both the random and stratified designs. Bootstrapping is a good measure of sampling variability or precision given the population data, but it does not fix bias in the population data if the study design is biased, like in the case of the summer-only design. Having a good study design can improve accuracy the accuracy of your data. 
...
>The confidence interval does contain the true population mean (35.4762). This tells me that the estimate is reasonably accurate. However, the confidence interval reveals the uncertainty in sampling as it provides a range ofplausible values for the true population mean, even though the population mean falls within the confidence interval in this instance.  
...
> The red vertical lines represent the 95% confidence interval, or to be more specific the lower and upper bounds. 
...
> The slope confidence interval does not include zero. Because the slope confidence interval is from -0.03468274 to -0.03180575 it suggests that their could be a negative linear relationship between `swc` and `le`. So this implies that as the predictor variable (`le`) increases, the response variable (`swc`) decreases.
